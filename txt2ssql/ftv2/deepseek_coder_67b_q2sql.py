#!/usr/bin/env python3
"""
DeepSeek-Coder 6.7B - Question to SQL (Direct Single-Stage)
Training script using DeepSeek's code-specialized model for SQL generation
"""
...
class Config:
    ...
    TARGET_REPO = "taherdoust/deepseek-coder-67b-cim-q2sql"
...
def create_prompt(sample: Dict) -> str:
    """Create DeepSeek-Coder style prompt with instruction format"""
...
Generate only the SQL query without explanations."""
    ...
    # DeepSeek-Coder format (instruction-style)
    prompt = f"""### System:
{system_msg}
...
def main():
    print("="*70)
    print("CIM Q2SQL Training - DeepSeek-Coder 6.7B (Direct)")
    print("="*70)
    ...
    # DeepSeek-Coder uses similar architecture to Llama
    ...
    output_dir = f"{config.CHECKPOINT_DIR}/deepseek-coder-67b-r{config.LORA_R}-q2sql"
    ...
    if config.WANDB_ENABLED:
        wandb.init(entity=config.WANDB_ENTITY, project=config.WANDB_PROJECT,
                  name=f"deepseek-coder-67b-r{config.LORA_R}-q2sql", config=vars(config))
