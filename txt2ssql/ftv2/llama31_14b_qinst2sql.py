#!/usr/bin/env python3
"""
Llama 3.1 14B - Question + Instruction to SQL Generator
"""
...
class Config:
    ...
    TARGET_REPO = "taherdoust/llama31-14b-cim-qinst2sql"
...
def main():
    print("="*70)
    print("CIM QInst2SQL Training - Llama 3.1 14B")
    print("="*70)
    ...
    output_dir = f"{config.CHECKPOINT_DIR}/llama31-14b-r{config.LORA_R}-qinst2sql"
    ...
    if config.WANDB_ENABLED:
        wandb.init(entity=config.WANDB_ENTITY, project=config.WANDB_PROJECT,
                  name=f"llama31-14b-r{config.LORA_R}-qinst2sql", config=vars(config))
