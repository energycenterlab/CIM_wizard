#!/usr/bin/env python3
"""
Qwen 2.5 32B - Question to SQL (Direct Single-Stage, Unsloth)
"""
...
class Config:
    ...
    TARGET_REPO = "taherdoust/qwen25-32b-unsloth-cim-q2sql"
...
def main():
    print("="*70)
    print("CIM Q2SQL Training - Qwen 2.5 32B (Direct, Unsloth)")
    print("="*70)
    ...
    output_dir = f"{config.CHECKPOINT_DIR}/qwen25-32b-unsloth-r{config.LORA_R}-q2sql"
    ...
    if config.WANDB_ENABLED:
        wandb.init(entity=config.WANDB_ENTITY, project=config.WANDB_PROJECT,
                  name=f"qwen25-32b-unsloth-r{config.LORA_R}-q2sql", config=vars(config))
