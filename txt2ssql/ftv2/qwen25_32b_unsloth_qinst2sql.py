#!/usr/bin/env python3
"""
Qwen 2.5 32B - Question + Instruction to SQL Generator (Unsloth)
"""
...
class Config:
    ...
    LOCAL_DATASET_PATH = "/media/space/castangia/Ali_workspace/curated_dataset_ftv2/qinst2sql_train.jsonl"
    CHECKPOINT_DIR = "/media/space/castangia/Ali_workspace/models/ftv2"
    TARGET_REPO = "taherdoust/qwen25-32b-unsloth-cim-qinst2sql"
...
Your task is to generate precise PostGIS spatial SQL queries based on natural language questions and reasoning instructions.
...
def main():
    print("="*70)
    print("CIM QInst2SQL Training - Qwen 2.5 32B (Unsloth)")
    print("="*70)
    ...
    output_dir = f"{config.CHECKPOINT_DIR}/qwen25-32b-unsloth-r{config.LORA_R}-qinst2sql"
    ...
    if config.WANDB_ENABLED:
        wandb.init(entity=config.WANDB_ENTITY, project=config.WANDB_PROJECT,
                  name=f"qwen25-32b-unsloth-r{config.LORA_R}-qinst2sql", config=vars(config))
