#!/usr/bin/env python3
"""
Mistral 7B v0.3 - Question to SQL (Direct Single-Stage)
Training script for direct question to SQL generation using Mistral architecture
"""
...
class Config:
    ...
    TARGET_REPO = "taherdoust/mistral-7b-cim-q2sql"
...
def create_prompt(sample: Dict) -> str:
    """Create Mistral-style prompt with [INST] tags"""
...
def main():
    print("="*70)
    print("CIM Q2SQL Training - Mistral 7B v0.3 (Direct)")
    print("="*70)
    ...
    # Mistral uses same attention projection modules as Llama
    ...
    output_dir = f"{config.CHECKPOINT_DIR}/mistral-7b-r{config.LORA_R}-q2sql"
    ...
    if config.WANDB_ENABLED:
        wandb.init(entity=config.WANDB_ENTITY, project=config.WANDB_PROJECT,
                  name=f"mistral-7b-r{config.LORA_R}-q2sql", config=vars(config))
