#!/usr/bin/env python3
"""
Qwen 2.5 14B - Question to SQL (Direct Single-Stage)
Training script for direct question to SQL generation
"""
...
class Config:
    ...
    LOCAL_DATASET_PATH = os.getenv('LOCAL_DATASET_PATH', "/media/space/castangia/Ali_workspace/nov/curated_q2sql/q2sql_train.jsonl")
    CHECKPOINT_DIR = os.getenv('CHECKPOINT_DIR', "/media/space/castangia/Ali_workspace/nov/models/")
    TARGET_REPO = "taherdoust/qwen25-14b-cim-q2sql-v3"
...
def main():
    parser = argparse.ArgumentParser(description='Fine-tune Qwen 2.5 14B on CIM Q2SQL dataset')
    parser.add_argument('--keep_checkpoints', type=int, default=3,
                       help='Number of checkpoints to keep (0 or -1 = keep all, N = keep N most recent). Default: 3')
    args = parser.parse_args()
    
    # Validate keep_checkpoints
    if args.keep_checkpoints < -1:
        parser.error("--keep_checkpoints must be >= -1 (use -1 or 0 to keep all)")
    
    print("="*70)
    print("CIM Q2SQL Training - Qwen 2.5 14B (Direct)")
    print("="*70)
    ...
    output_dir = f"{config.CHECKPOINT_DIR}/qwen25-14b-r{config.LORA_R}-q2sql"
    ...
    training_args = TrainingArguments(
        ...
        save_strategy="steps", save_steps=eval_steps, save_total_limit=save_total_limit,
    ...
    if config.WANDB_ENABLED:
        wandb.init(entity=config.WANDB_ENTITY, project=config.WANDB_PROJECT,
                  name=f"qwen25-14b-r{config.LORA_R}-q2sql", config=vars(config))
